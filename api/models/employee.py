...
class Employee(Model, ActivatableModel, CreateableModel):
    ...
    manager = ForeignKey('Employee', related_name='supervised_employees', on_delete=DO_NOTHING, null=True, blank=True)
    office = ForeignKey('Office', related_name='employees', on_delete=DO_NOTHING)
    current_week_start = DateField()
    position = ManyToManyField('BillRate')
